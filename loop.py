...
def trigger_report_function(report_id):
    ...
    polls_for_each_store = store_status.groupby('store_id')
    ...
    for store in store_IDs:
        ...
        # Convert business hours start and end timestamps, for the past 7 days from current timestamp, to UTC
        ...
        for i in range(8):
            ...
            if ((store_hours['store_id'].isin([store])) & (store_hours['day'].isin([day]))).any():
                business_24x7 = False
                business_hours = store_hours.loc[
                    (store_hours['store_id'].isin([store])) & (store_hours['day'].isin([day]))]
            ...
            if business_24x7:
                business_start_timestamp = timestamp_in_store_tz.normalize()
                business_end_timestamp = timestamp_in_store_tz.normalize() + datetime.timedelta(days=1)
            else:
                business_start_timestamp = timestamp_in_store_tz.normalize() + datetime.timedelta(
                    hours=business_start_time.hour,
                    minutes=business_start_time.minute,
                    seconds=business_start_time.second
                )
                business_end_timestamp = timestamp_in_store_tz.normalize() + datetime.timedelta(
...
def trigger_report():
    ...
    # Start job in a different thread
